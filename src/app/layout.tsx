...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return {
    ...
    description:
      ...
      "האתר שמביא לך את כל המידע על רכבים, טכנולוגיות חדשות וטיפים לנהיגה חכמה.",
    keywords: siteInfo?.meta_keywords?.split(", ") || [
      "רכבים",
      "חדשות רכב",
      "ביקורות רכב",
      "תחזוקת רכב",
    ...
    openGraph: {
      title:
        siteInfo?.og_title ||
        siteInfo?.meta_title ||
        "עולם הרכב - כל מה שצריך לדעת על רכבים",
      description:
        siteInfo?.og_description ||
        siteInfo?.meta_description ||
        "גלה את כל מה שחדש ומעניין בתחום הרכב, כולל ביקורות, טיפים, וטכנולוגיות חדשות.",
      ...
      images: [
        {
          url:
            siteInfo?.og_image ||
            "https://example.com/default-og-image-car.jpg",
          width: 800,
          height: 600,
    ...
    twitter: {
      ...
      description:
        siteInfo?.og_description ||
        siteInfo?.meta_description ||
        "כל המידע על רכבים, טכנולוגיות חדשות, ביקורות ועוד.",
      images: [
        siteInfo?.og_image ||
...
export default async function RootLayout({
  ...
  const menuItems = sortedPages?.map((page: any) => ({
    href: "/" + page.slug,
    label: page.name || page.title || page.slug,
  }));
